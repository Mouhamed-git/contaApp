pipeline {
    ...
    stages {
        ...
        stage ('Deploy') {
           steps {
               sshagent(['nginx']) {
                   sh 'scp -o StrictHostKeyChecking=no -r dist/** ubuntu@ec2-35-172-134-38.compute-1.amazonaws.com:~/'
               }
           }
       }
       ...
       stage ('DAST') {
           steps {
               sshagent(['zap']) {
                   sh 'ssh -o StrictHostKeyChecking=no ubuntu@ec2-35-172-134-38.compute-1.amazonaws.com "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://44.201.89.145/" || true'
               }
           }
       }
